...
class SOLStopLossStrategy:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        self.profit_threshold = decimal.Decimal('0.015')  # 止盈阈值 1.5%
        self.stop_loss_threshold = decimal.Decimal('0.01')  # 止损阈值 1%
        self.min_holding_time = 3600  # 最小持仓时间 1小时
    ...
    async def check_exit_conditions(self) -> Optional[str]:
        ...
        # 检查最小持仓时间后的盈利退出
        if holding_hours >= 1.0 and pnl_percentage > decimal.Decimal('0.005'):  # 0.5%盈利
            return f"持仓1小时且有盈利，获利了结 (盈利{pnl_percentage*100:.2f}%)"
